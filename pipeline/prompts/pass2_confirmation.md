...
5. **Headlines vs body: the body governs when they disagree.** Cap the verdict at `rumored` when the body *hedges against* a stronger headline (headline says "Launch" / "Deal Done" but the body describes only a plan, a report, or speculation), or when the event appears only as a passing headline mention that the body never discusses or substantiates. But a headline's factual claim that the body *supports with consistent detail* (figures, specifics of the event) counts as stated fact even if the body doesn't repeat the claim word-for-word — "Company Beats Estimates" over a body of strong results is a confirmation, not a rumor.
6. **Match the catalyst as written.** If the user is waiting for "hyperscaler capex cut" and the article covers a capex *increase*, that is `no_change` (or `invalidated` if a cut had been reported and is now contradicted) — not a loose topical match.
7. `confidence` is your honest probability (0.0–1.0) that your proposed state is right. `reasoning` is one or two plain sentences the user will read in the dashboard — say what the article establishes and why it does or doesn't move the catalyst.
